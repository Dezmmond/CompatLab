...
def _command_warning(path: Path, result: CommandResult) -> Problem:
...
def _apply_result(
    ...
    warnings.append(_command_warning(path, result))
...
def scan_path(path: Path) -> ArtifactReport:
    ...
    header_output = _apply_result(path, warnings, run_readelf(["-h"], path), "readelf -h")
    if header_output is not None:
        elf = elf.model_copy(update=parse_elf_header(header_output))

    program_headers = _apply_result(path, warnings, run_readelf(["-l"], path), "readelf -l")
    if program_headers is not None:
        elf = elf.model_copy(update=parse_program_headers(program_headers))

    dynamic_output = _apply_result(path, warnings, run_readelf(["-d"], path), "readelf -d")
    ...
    version_output = _apply_result(
        path,
        warnings,
        run_readelf(["--version-info"], path),
